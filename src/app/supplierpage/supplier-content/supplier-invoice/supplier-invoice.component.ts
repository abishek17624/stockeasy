...
export class SupplierInvoiceComponent implements OnInit{
  ...
  downloadInvoice(invoiceNumber: string): void {
    alert(`Downloading invoice ${invoiceNumber} as PDF`);
    this.showToast(`Invoice ${invoiceNumber} download started`, 'success');
  }

  // Mark invoice as paid
  markAsPaid(): void {
    if (this.selectedInvoice) {
      const invoice = this.invoiceData.find(inv => inv.id === this.selectedInvoice?.id);
      if (invoice) {
        invoice.status = 'paid';
        this.updateStats();
        this.filterInvoices();
        this.showToast(`Invoice ${invoice.id} marked as paid`, 'success');
  ...
  sendReminder(): void {
    if (this.selectedInvoice) {
      this.showToast(`Reminder sent for invoice ${this.selectedInvoice.id}`, 'success');
    }
  }
